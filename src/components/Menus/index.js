...
export default class MainMenu extends React.PureComponent {
        ...
        },
        {
          ...
          children: [
            ...
            },
            {
              ...
              children: [
                {
                  key: "first_four_first",
                  value: "1_4_1",
                  icon: "pay-circle-o",
                  url: "urls.First_Four_First",
                  children: null
                }
              ]
            }
    ...
    const loop = (data = []) =>
      data.map(item => {
        if (item.children) {
          ...
          return (
            <SubMenu key={item.key} title={subTitle}>
              {loop(item.children)}
            </SubMenu>
          );
        }
        return (
          <MenuItem
            key={item.key}
            className={this.getMenuItemClass(urls[item.url.split(".")[1]])}
          >
  ...
  render() {
    ...
    return menusData.length > 0 ? (
      <Menu theme="dark" mode={this.state.mode} style={{ border: "none" }}>
        {menusData}
      </Menu>
    ) : null;
  }
